...
func (s *scenarioState) initializeScenario(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^the candidate is configured with PUID (\d+) and PGID (\d+)$`, s.configureRequestedIdentity)
	...
	ctx.Step(`^the persistence data still belongs to UID and GID (\d+)$`, s.persistenceUnchangedFor)
	ctx.Step(`^the persistence data belongs to UID (\d+) and GID (\d+)$`, s.persistenceBelongsTo)
	ctx.Step(`^Montainer PID 1 and Bedrock run as UID (\d+) and GID (\d+)$`, s.candidateProcessesRunAsIdentity)
