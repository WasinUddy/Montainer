...
1. one runner verifies the recorded URL and SHA-256, builds the Mojang-backed image, and pushes an untagged immutable digest;
...
Manual workflow dispatches default to validation-only and leave the candidate untagged; a maintainer must explicitly enable the `publish` input on `main` to promote a manually dispatched build.
